 ...
 * @returns {Array} 公开日志数组
 */
export const getPublicJournals = (userId = null) => {
  let publicJournals = journals.filter(j => j.is_public === true);
  if (userId) {
    publicJournals = publicJournals.filter(j => j.user_id === userId);
  }
  return publicJournals.sort((a, b) => new Date(b.date) - new Date(a.date));
};
 ...
 * @returns {Array} 日志数组
 */
export const getJournalsByMood = (mood) => {
  return journals
    .filter(j => j.mood === mood)
    .sort((a, b) => new Date(b.date) - new Date(a.date));
};
...
/**
 * 获取所有心情类型
 * @returns {Array} 心情类型数组
 */
export const getMoodTypes = () => {
  return ['excited', 'happy', 'calm', 'thoughtful', 'motivated', 'sad', 'anxious', 'neutral'];
};
...
export const getWeatherTypes = () => {
  return ['sunny', 'cloudy', 'rainy', 'snowy', 'windy', 'clear', 'stormy'];
};
